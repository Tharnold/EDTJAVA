...
public class ModifierSeanceSalle extends Frame implements ActionListener{
   
	JFrame modifSeanceSalle=new JFrame();
	...
	public ModifierSeanceSalle()
	{
		super("Modifier une nouvelle seance :");
		modifSeanceSalle.setLayout(new BorderLayout());
		...
		modifSeanceSalle.add(ptitre,BorderLayout.NORTH);
		modifSeanceSalle.add(panelText,BorderLayout.CENTER);
		panelButton.add(valider);panelButton.add(annuler);
		modifSeanceSalle.add(panelButton,BorderLayout.SOUTH);
		modifSeanceSalle.setVisible(true);
		modifSeanceSalle.pack();
	...
	public void actionPerformed(ActionEvent e) {
		...
		{	
			...
			Statement stm=cnx.createStatement ();
					...
					stm.executeUpdate("update seance_salle set IDséance = '"+IDséance1+"'where ID ='"+IDs+"'"); 
                                        stm.executeUpdate("update seance_salle set IDgroupe = '"+IDsalle1+"'  where ID ='"+IDs+"'");  
				stm.close();
				cnx.close ();
				//JOptionPane.showMessageDialog(AjouteSeance, "seance ajouté avec succé", "Ajouter",JOptionPane.INFORMATION_MESSAGE) ;
				modifSeanceSalle.dispose();
				}
				catch(Exception ex)
				{
					JOptionPane.showMessageDialog(modifSeanceSalle, "Erreur d'ajout verifier les champs", "Erreur",JOptionPane.ERROR_MESSAGE) ;
				}
